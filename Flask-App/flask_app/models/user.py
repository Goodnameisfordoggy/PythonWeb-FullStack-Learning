...
class User(db.Model):
    ...
    PROTECTED_FIELDS = {"id", "user_identity", "create_time"}
    ...
    def delete(cls, user_identity):
        ...
        try:
            # 删除操作
            affected_rows = cls.query.filter_by(user_identity=user_identity, is_deleted=0).update({"is_deleted": 1})
            db.session.commit()
            # 根据受影响行数判断是否删除成功
            if affected_rows > 0:
                return True, "删除成功"
    ...
    def update_by_identity(cls, identity, **kwargs):
        ...
        try:
            ...
            # 更新操作
            affected_rows = cls.query.filter_by(user_identity=identity).update(update_data)
            db.session.commit()
            # 根据受影响行数判断是否更新成功
            if affected_rows > 0:
                return True, "更新成功"
    ...
    def get_by_user_identity(cls, user_identity):
        """通过唯一标识查询用户"""
        return cls.query.filter_by(user_identity=user_identity).first()
