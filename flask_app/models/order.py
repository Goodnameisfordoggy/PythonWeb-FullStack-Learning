...
class Order(db.Model):
    ...
    def delete(cls, order_identity):
        ...
        try:
            affected_rows = cls.query.filter_by(
                order_identity=order_identity, is_deleted=0
            ).update({"is_deleted": 1})
            db.session.commit()
            # 根据受影响行数判断是否删除成功
            if affected_rows > 0:
    ...
    def get_by_order_identity(cls, order_identity):
        """通过订单唯一标识查询订单（排除已删除）"""
        return cls.query.filter_by(order_identity=order_identity, is_deleted=0).first()

    @classmethod
    def get_by_user_identity(cls, user_identity):
        """查询指定用户的所有有效订单"""
        return cls.query.filter_by(user_identity=user_identity, is_deleted=0).all()
